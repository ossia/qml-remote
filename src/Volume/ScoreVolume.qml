...
ScoreSliderSkeleton {
    ...
    onMoved: {
        socket.sendTextMessage(('{ "Message": "IntervalGain", "Path":'.concat(
                                    scoreGlobalVolume.path, ', "Gain": ',
                                    scoreGlobalVolume.value, '}')))
    }
    ...
    Connections {
        ...
        function onIntervalsMessageReceived(m) {
            ...
            if (IntervalsObject[0] && scoreGlobalVolume.path === JSON.stringify(
                ...
                // The global path is the first one to be created by score
                scoreGlobalVolume.value = 1 - JSON.stringify(
                            IntervalsObject[0].Gain)
            }
        }
    }
}
